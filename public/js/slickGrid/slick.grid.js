...
function toggleObj(o,s) {

  for (var a in o) {
    if (typeof o[a] == 'object'){
      return toggleObj(o[a]);
    }else{
         $("#myImage").append(a);
 ...
 function formatJson(json) {
        ...
        for (i = 0, il = json.length; i < il; i += 1) { 
            ...
            case '[': 
                if (!inString) { 
                    indentLevel -= 1; 
                    newJson += ""+repeat(tab, indentLevel); 
                } else { 
        ...
        return htmlsafe(newJson); 
    }


        function updateOnto(rows) {
                    ...
                    var uri = d.finish;
                        ...
                        $.ajax({
                            ...
                            data=JSON.parse(data);
                              ...
                              var img = "<h5>"+uri+"</h5><br>";
                              
                              var image = data.image;//JSON.stringify();
                              for(var idx in image){
                                 var path = image[idx].split("\/");
                               // if(image[idx].match(/\/\/upload/i) &&  !image[idx].match(/(logo|Edit|clear|wikimediafoundation|Question|book)/i)){          
                                  img += "<img style='margin:10px;padding:10px;border:#000 1px solid;' src='"+image[idx]+"' alt='"+path[path.length-1]+"'>";
...
/*
                             if(data.audio) {
                               var med = "<h5>"+uri +"</h5><br>";
                               
                                var media = data.audio;//JSON.stringify();
                                for(var idx in media){
                                   var path = media[idx].split("\/");
                                  if(media[idx].match(/\/\/upload/i)){

                                    if(media[idx].match(/ogg/)){
                                      med += '<li><audio controls><source src="'+media[idx]+'" type="audio/ogg"></audio><br>http:'+media[idx]+'</li>';
                                    }
                                    if(media[idx].match(/ogv/)){
                                      med += '<li><video id="movies" onmouseover="this.play()" onmouseout="this.pause()" autobuffer="true" width="400px" height="300px"><source src="http:'+media[idx]+'" type="video/ogg;"></video><br>http:'+media[idx]+'</li>';
...
*/
                              ...
                              if(data.links) {
                                ...
                                var med = "<h5>"+uri +"</h5><br>";
                                media = data.links;//JSON.stringify();
                                for(var idx in media){
                                   var path = media[idx].split("\/");
                                  if(media[idx].match(/(\/\/upload|http).*.(ogg|ogv)$/i)){          
                              ...
                              for(var idx in data.pdf){
                                s += "<a href='"+data.pdf[idx]+"' target='_blank'><img src='http://localhost/docs/assets/img/pdf.png' width='16px' height='16px' ></a>";
                                pdfI++;
                              }
                              ...
                              if(data.article){
                                 ...
                                 //;=+formatJson(data.article)+"<p>""</p><hr>");
                                 var xur =  htmlsafe(uri);
                                 var mtext = "<h5>"+uri+"</h5>";
                                 xur = encodeURIComponent(uri);
                                 mtext +='<div class="tabbable">   <ul class="nav nav-tabs">'+
                                 '<li class="active" ><a href="#'+xur+'tab1" data-toggle="tab">Article</a></li>';

                                 if(pdfI)
                                  mtext+='<li><a href="#'+xur+'tab2" data-toggle="tab">pdf '+pdfI+'</a></li>';
                                 
                                 if(imgI)
                                  mtext+='<li><a href="#'+xur+'tab3" data-toggle="tab">image '+imgI+'</a></li>';

                                 if(medI)
                                  mtext+='<li><a href="#'+xur+'tab4" data-toggle="tab">media '+medI+'</a></li>';

                                  mtext +='<li><a href="#'+xur+'tab5" data-toggle="tab">relation</a></li></ul></div> </div>'+
                                 '<div class="tab-content">    <div class="tab-pane active" id="'+xur+'tab1"><br><p>'+
                                 formatJson(data.article)+'</p><hr></div>   '+
                                 '<div class="tab-pane " id="'+xur+'tab2">'+
                                 '<p>'+s+'</p>    </div>'+
'<div class="tab-pane " id="'+xur+'tab3">'+
                                 '<p>'+img+'</p>    </div>'+
                                 
'<div class="tab-pane " id="'+xur+'tab4">'+
                                 '<p>'+med+'</p></div>'+
'<div class="tab-pane " id="'+xur+'tab5"></div></div>';
                                   $("#myText").prepend(mtext);
                              }
